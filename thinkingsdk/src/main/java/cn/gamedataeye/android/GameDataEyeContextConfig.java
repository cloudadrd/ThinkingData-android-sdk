 ...
 */
class GameDataEyeContextConfig {
    private static final String KEY_MAIN_PROCESS_NAME = "cn.gamedataeye.android.MainProcessName";
    ...
    // 是否打开日志
    private static final String KEY_ENABLE_LOG = "cn.gamedataeye.android.EnableTrackLogging";
    // 设置数据保留天数，默认 15 天
    private static final String KEY_RETENTION_DAYS = "cn.gamedataeye.android.RetentionDays";
    // 数据库文件最小大小，单位 Mb; 当系统空间不足时，缓存数据库达到此上限后会删除最老的 100 条数据.
    private static final String KEY_MIN_DB_LIMIT = "cn.gamedataeye.android.MinimumDatabaseLimit";
    // 是否允许退出时等待工作线程安全退出，默认允许
    private static final String KEY_ENABLE_QUIT_SAFELY = "cn.gamedataeye.android.EnableQuitSafely";
    // 在允许退出时等待工作线程安全退出的情况下，请求超时时长
    private static final String KEY_QUIT_SAFELY_TIMEOUT = "cn.gamedataeye.android.QuitSafelyTimeout";

    private final static  Map<Context, GameDataEyeContextConfig> sInstanceMap = new HashMap<>();
    ...
    static GameDataEyeContextConfig getInstance(Context context) {
        synchronized (sInstanceMap) {
            GameDataEyeContextConfig contextConfig = sInstanceMap.get(context);
            if (null == contextConfig) {
                contextConfig = new GameDataEyeContextConfig(context);
    ...
    private GameDataEyeContextConfig(Context context) {
