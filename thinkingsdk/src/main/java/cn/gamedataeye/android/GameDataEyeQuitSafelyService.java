...
public class GameDataEyeQuitSafelyService {
    static final String TAG = "DataEyeAnalytics.Quit";

    private static GameDataEyeQuitSafelyService sInstance;
    private Context mContext;
    private boolean mExceptionHandlerInitialed;

    private GameDataEyeQuitSafelyService(Context context) {
        mContext = context.getApplicationContext();
        if (GameDataEyeContextConfig.getInstance(mContext).quitSafelyEnabled()) {
     ...
     */
    static GameDataEyeQuitSafelyService getInstance(Context context) {
        if (sInstance == null) {
            if (context == null) return null;
            synchronized (ExceptionHandler.class) {
                if (sInstance == null) {
                    sInstance = new GameDataEyeQuitSafelyService(context);
                }
            }
        }
        return sInstance;
    ...
    private void quit() {
        if (GameDataEyeContextConfig.getInstance(mContext).quitSafelyEnabled()) {
            ...
            GameDataEyeAnalyticsSDK.allInstances(new GameDataEyeAnalyticsSDK.InstanceProcessor() {
                @Override
                public void process(GameDataEyeAnalyticsSDK dataEyeAnalyticsSDK) {
                    dataEyeAnalyticsSDK.flush();
                }
            });

            quitSafely(GameDataEyeDataHandle.THREAD_NAME_SAVE_WORKER, GameDataEyeContextConfig.getInstance(mContext).getQuitSafelyTimeout());
            quitSafely(GameDataEyeDataHandle.THREAD_NAME_SEND_WORKER, GameDataEyeContextConfig.getInstance(mContext).getQuitSafelyTimeout());
    ...
    private class ExceptionHandler implements Thread.UncaughtExceptionHandler {
        ...
        public void uncaughtException(final Thread t, final Throwable e) {
            ...
            Throwable cause = e;
            while (null != cause) {
                if (cause instanceof GameDataEyeDebugException) {
