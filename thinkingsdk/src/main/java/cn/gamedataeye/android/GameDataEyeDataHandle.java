...
import cn.gamedataeye.android.utils.HttpService;
import cn.gamedataeye.android.utils.RemoteService;
 ...
 */
public class GameDataEyeDataHandle {
    ...
    private final GameDataEyeSystemInformation mGameDataEyeSystemInformation;
    private final GameDataEyeDatabaseAdapter mDbAdapter;
    private final Context mContext;

    private static final Map<Context, GameDataEyeDataHandle> sInstances = new HashMap<>();
     ...
     */
    static GameDataEyeDataHandle getInstance(final Context messageContext) {
        synchronized (sInstances) {
            final Context appContext = messageContext.getApplicationContext();
            final GameDataEyeDataHandle ret;
            if (!sInstances.containsKey(appContext)) {
                ret = new GameDataEyeDataHandle(appContext);
    ...
    GameDataEyeDataHandle(final Context context) {
        mContext = context.getApplicationContext();
        GameDataEyeContextConfig config = GameDataEyeContextConfig.getInstance(mContext);
        mGameDataEyeSystemInformation = GameDataEyeSystemInformation.getInstance(mContext);
        mDbAdapter = getDbAdapter(mContext);
        mDbAdapter.cleanupEvents(System.currentTimeMillis() - config.getDataExpiration(), GameDataEyeDatabaseAdapter.Table.EVENTS);
    ...
    protected GameDataEyeDatabaseAdapter getDbAdapter(Context context) {
        return GameDataEyeDatabaseAdapter.getInstance(context);
    }

    // for auto tests.
    protected GameDataEyeConfig getConfig(String token) {
        return GameDataEyeConfig.getInstance(mContext, token);
    }

    /**
     * 保存数据到本地数据库
     */
    void saveClickData(final GameDataEyeDataDescription gameDataEyeDataDescription) {
        mSaveMessageWorker.saveToDatabase(gameDataEyeDataDescription);
    }

    /**
     * 立即上报到服务器，不会缓存和重试
     */
    void postClickData(final GameDataEyeDataDescription gameDataEyeDataDescription) {
        mSendMessageWorker.postToServer(gameDataEyeDataDescription);
    }

    /**
     * Debug 模式上报数据，逐条上报
     */
    void postToDebug(final GameDataEyeDataDescription gameDataEyeDataDescription) {
        mSendMessageWorker.postToDebug(gameDataEyeDataDescription);
    }
    ...
    private class SaveMessageWorker {
        ...
        void saveToDatabase(final GameDataEyeDataDescription gameDataEyeDataDescription) {
            final Message msg = Message.obtain();
            msg.what = ENQUEUE_EVENTS;
            msg.obj = gameDataEyeDataDescription;
            if (null != mHandler) {
        ...
        private class AnalyticsSaveMessageHandler extends Handler {
            ...
            public void handleMessage(Message msg) {
                if (msg.what == ENQUEUE_EVENTS) {
                    try {
                        int ret;
                        GameDataEyeDataDescription gameDataEyeDataDescription = (GameDataEyeDataDescription) msg.obj;
                        if (null == gameDataEyeDataDescription) return;
                        String token = gameDataEyeDataDescription.mToken;
                        if (removingTokens.contains(token)) {
                            return;
                        }

                        JSONObject data = gameDataEyeDataDescription.get();
                        try {
                        ...
                        synchronized (mDbAdapter) {
                            ret = mDbAdapter.addJSON(data, GameDataEyeDatabaseAdapter.Table.EVENTS, token);
                        }
                        if (ret < 0) {
                    ...
                    synchronized (mDbAdapter) {
                        mDbAdapter.cleanupEvents(GameDataEyeDatabaseAdapter.Table.EVENTS, (String) msg.obj);
                    }
    ...
    protected int getFlushBulkSize(String token) {
        GameDataEyeConfig config = getConfig(token);
        return null == config ? GameDataEyeConfig.DEFAULT_FLUSH_BULK_SIZE : config.getFlushBulkSize();
    }

    protected int getFlushInterval(String token) {
        GameDataEyeConfig config = getConfig(token);
        return null == config ? GameDataEyeConfig.DEFAULT_FLUSH_INTERVAL : config.getFlushInterval();
    }
    ...
    private class SendMessageWorker {
        ...
        SendMessageWorker() {
            ...
            mPoster = getPoster();
            mDeviceInfo = new JSONObject(mGameDataEyeSystemInformation.getDeviceInfo());
        }
        ...
        // 立即发送数据, 没有重试
        void postToServer(GameDataEyeDataDescription gameDataEyeDataDescription) {
            if (null == gameDataEyeDataDescription) return;
            Message msg = Message.obtain();
        ...
        void postToDebug(GameDataEyeDataDescription gameDataEyeDataDescription) {
            if (null == gameDataEyeDataDescription) return;
            Message msg = Message.obtain();
        ...
        private class AnalyticsMessageHandler extends Handler {
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case FLUSH_QUEUE: {
                        String token = (String) msg.obj;
                        final GameDataEyeConfig config = getConfig(token);
                        if (null == config) {
                    ...
                    case FLUSH_QUEUE_OLD: {
                        final GameDataEyeConfig config = getConfig((String) msg.obj);
                        if (null == config) {
                            DataEyeLog.w(TAG, "Could found config object for token. Canceling...");
                            return;
                        }
                    ...
                    case SEND_TO_SERVER:
                        try {
                            ...
                            JSONObject data = gameDataEyeDataDescription.get();
                            sendData(getConfig(gameDataEyeDataDescription.mToken), data);
                        } catch (Exception e) {
                    ...
                    case SEND_TO_DEBUG: {
                        try {
                            GameDataEyeDataDescription gameDataEyeDataDescription = (GameDataEyeDataDescription) msg.obj;
                            if (null == gameDataEyeDataDescription) return;
                            GameDataEyeConfig config = getConfig(gameDataEyeDataDescription.mToken);
                            if (config.isNormal()) {
                                saveClickData(gameDataEyeDataDescription);
                            } else {
                                try {
                                    JSONObject data = gameDataEyeDataDescription.get();
                                    if (gameDataEyeDataDescription.mType.isTrack()) {
                                    ...
                                    if (config.shouldThrowException()) {
                                        throw new GameDataEyeDebugException(e);
        ...
        // 发送单条数据到 Debug 模式
        private void sendDebugData(GameDataEyeConfig config, JSONObject data) throws IOException, RemoteService.ServiceUnavailableException, JSONException {
            ...
            if (errorLevel == -1) {
                ...
                config.setMode(GameDataEyeConfig.ModeEnum.NORMAL);
                throw new GameDataEyeDebugException("Fallback to normal mode due to the device is not allowed to debug for: " + tokenSuffix);
            }
            ...
            if (errorLevel != 0) {
                ...
                if (config.shouldThrowException()) {
                    if (1 == errorLevel) {
                        throw new GameDataEyeDebugException("Invalid properties. Please refer to the logcat log for detail info.");
                    } else if (2 == errorLevel) {
                        throw new GameDataEyeDebugException("Invalid data format. Please refer to the logcat log for detail info.");
                    } else {
                        throw new GameDataEyeDebugException("Unknown error level: " + errorLevel);
                    }
                }
            } else {
        ...
        // 发送单条数据到接收端
        private void sendData(GameDataEyeConfig config, JSONObject data) throws IOException, RemoteService.ServiceUnavailableException, JSONException {
        ...
        private void sendData(GameDataEyeConfig config) {
            sendData(config.mToken, config);
        }

        private void sendData(String fromToken, GameDataEyeConfig config) {
            if (config == null) {
            ...
            try {
                if (!mGameDataEyeSystemInformation.isOnline()) {
                    return;
                }

                String networkType = mGameDataEyeSystemInformation.getNetworkType();
            ...
            do {
                ...
                String[] eventsData;
                synchronized (mDbAdapter) {
                    eventsData = mDbAdapter.generateDataString(GameDataEyeDatabaseAdapter.Table.EVENTS, fromToken, 50);
                }
                if (eventsData == null) {
                    return;
                }
                ...
                try {
                    ...
                    if (deleteEvents) {
                        synchronized (mDbAdapter) {
                            count = mDbAdapter.cleanupEvents(lastId, GameDataEyeDatabaseAdapter.Table.EVENTS, fromToken);
                        }
        ...
        private Map<String, String> createExtraHeaders(String count) {
            ...
            extraHeaders.put(INTEGRATION_TYPE, GameDataEyeSystemInformation.getLibName());
            extraHeaders.put(INTEGRATION_VERSION, GameDataEyeSystemInformation.getLibVersion());
