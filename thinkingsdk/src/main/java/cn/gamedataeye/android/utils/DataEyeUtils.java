...
import cn.gamedataeye.android.GameDataEyePathFinder;
    ...
    private static int getChildIndex(ViewParent parent, View child) {
        try {
            ...
            for (int i = 0; i < _parent.getChildCount(); i++) {
                ...
                if (!GameDataEyePathFinder.hasClassName(brother, childClassName)) {
                    continue;
                }
    ...
    public static String getTitleFromFragment(final Object fragment, final String token) {
        ...
        try {
            if (fragment instanceof GameDataEyeScreenAutoTracker) {
                ...
                JSONObject trackProperties = gameDataEyeScreenAutoTracker.getTrackProperties();
            ...
            if (TextUtils.isEmpty(title) && fragment.getClass().isAnnotationPresent(GameDataEyeFragmentTitle.class)) {
                GameDataEyeFragmentTitle gameDataEyeFragmentTitle = fragment.getClass().getAnnotation(GameDataEyeFragmentTitle.class);
                if (gameDataEyeFragmentTitle != null) {
                    if (TextUtils.isEmpty(gameDataEyeFragmentTitle.appId()) ||
                            token.equals(gameDataEyeFragmentTitle.appId())) {
                        title = gameDataEyeFragmentTitle.title();
                    }
                }
            }
        } catch (Exception ex) {
