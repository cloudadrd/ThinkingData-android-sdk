...
import cn.gamedataeye.android.persistence.*;
import cn.gamedataeye.android.utils.ICalibratedTime;
import cn.gamedataeye.android.utils.ITime;
import cn.gamedataeye.android.utils.PropertyUtils;
import cn.gamedataeye.android.utils.DataEyeCalibratedTime;
import cn.gamedataeye.android.utils.DataEyeCalibratedTimeWithNTP;
import cn.gamedataeye.android.utils.DataEyeConstants;
import cn.gamedataeye.android.utils.DataEyeLog;
import cn.gamedataeye.android.utils.DataEyeTime;
import cn.gamedataeye.android.utils.DataEyeTimeCalibrated;
import cn.gamedataeye.android.utils.DataEyeTimeConstant;
import cn.gamedataeye.android.utils.DataEyeUtils;
import cn.gamedataeye.android.R;
...
public class GameDataEyeAnalyticsSDK implements GameDataEyeAnalyticsAPI {
     ...
     * @return SDK 实例
     */
    public static GameDataEyeAnalyticsSDK sharedInstance(Context context, String appId) {
     ...
     * @return SDK 实例
     */
    public static GameDataEyeAnalyticsSDK sharedInstance(Context context, String appId, String url) {
     ...
     */
    public static GameDataEyeAnalyticsSDK sharedInstance(Context context, String appId, String url, boolean trackOldData) {
        if (null == context) {
        ...
        GameDataEyeConfig config;
        try {
            config = GameDataEyeConfig.getInstance(context, appId, url);
    ...
    public static GameDataEyeAnalyticsSDK sharedInstance(GameDataEyeConfig config) {
        if (null == config) {
        ...
        synchronized (sInstanceMap) {
            ...
            Map<String, GameDataEyeAnalyticsSDK> instances = sInstanceMap.get(config.mContext);

            if (null == instances) {
                instances = new HashMap<>();
                sInstanceMap.put(config.mContext, instances);
                if (GameDataEyeDatabaseAdapter.dbNotExist(config.mContext)
                        && GameDataEyeSystemInformation.getInstance(config.mContext).hasNotBeenUpdatedSinceInstall()) {
                    sAppFirstInstallationMap.put(config.mContext, new LinkedList<String>());
                }
                GameDataEyeQuitSafelyService.getInstance(config.mContext).start();
            }

            GameDataEyeAnalyticsSDK instance = instances.get(config.mToken);
            if (null == instance) {
                instance = new GameDataEyeAnalyticsSDK(config);
    ...
    static void addInstance(GameDataEyeAnalyticsSDK instance, Context context, String appId) {
        synchronized (sInstanceMap) {
            Map<String, GameDataEyeAnalyticsSDK> instances = sInstanceMap.get(context);
            if (null == instances) {
                instances = new HashMap<>();
    ...
    private static boolean isOldDataTracked() {
        synchronized (sInstanceMap) {
            if (sInstanceMap.size() > 0) {
                for (Map<String, GameDataEyeAnalyticsSDK> instanceMap : sInstanceMap.values()) {
                    for (GameDataEyeAnalyticsSDK instance : instanceMap.values()) {
    ...
    protected GameDataEyeDataHandle getDataHandleInstance(Context context) {
        return GameDataEyeDataHandle.getInstance(context);
    }
     ...
     */
     GameDataEyeAnalyticsSDK(GameDataEyeConfig config, boolean... light) {
        mConfig = config;
        ...
        if (light.length > 0 && light[0]) {
            ...
            mGameDataEyeSystemInformation = GameDataEyeSystemInformation.getInstance(config.mContext);
            return;
        }
        ...
        //设置aid到accountid
        mLoginId.put(mGameDataEyeSystemInformation.getAndroidID(config.mContext));
        ...
        mLifecycleCallbacks = new GameDataEyeActivityLifecycleCallbacks(this, mConfig.getMainProcessName());
        ...
        DataEyeLog.i(TAG, String.format("Thinking Analytics SDK %s instance initialized successfully with mode: %s, APP ID ends with: %s, server url: %s, device ID: %s", GameDataEyeConfig.VERSION,
    ...
    public static void setCustomerLibInfo(String libName, String libVersion) {
        GameDataEyeSystemInformation.setLibraryInfo(libName, libVersion);
    }
        ...
        try {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                for (Iterator iterator = properties.keys(); iterator.hasNext(); ) {
                    ...
                    if (key.equals(DataEyeConstants.KEY_ACCOUNT_ID) || key.equals(DataEyeConstants.KEY_DISTINCT_ID) || mGameDataEyeSystemInformation.getDeviceInfo().containsKey(key)) {
                        iterator.remove();
                    }
                }

                GameDataEyeDataDescription gameDataEyeDataDescription;
                if (type.isTrack()) {
                    ...
                    gameDataEyeDataDescription = new GameDataEyeDataDescription(this, type, properties, time);
                    trackInternal(gameDataEyeDataDescription);
                }
            }
        } catch (Exception e) {
    ...
    private void track(String eventName, JSONObject properties, ITime time, boolean doFormatChecking, Map<String, String> extraFields, DataEyeConstants.DataType type) {
        ...
        try {
            if(doFormatChecking && PropertyUtils.isInvalidName(eventName)) {
                ...
                if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("Invalid event name: " + eventName);
            }
            ...
            if (doFormatChecking && !PropertyUtils.checkProperty(properties)) {
                ...
                if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("Invalid properties. Please refer to SDK debug log for detail reasons.");
            }
            ...
            GameDataEyeDataDescription gameDataEyeDataDescription = new GameDataEyeDataDescription(this, dataType, finalProperties, time);
            gameDataEyeDataDescription.eventName = eventName;
            if (null != extraFields) {
                gameDataEyeDataDescription.setExtraFields(extraFields);
            }
    ...
    @Override
    public void track(GameDataEyeAnalyticsEvent event) {
        if (hasDisabled()) return;
        if (null == event) {
        ...
        if (TextUtils.isEmpty(event.getExtraField())) {
            ...
            String extraValue;
            if (event instanceof GameDataEyeFirstEvent && event.getExtraValue() == null) {
    ...
    void trackInternal(GameDataEyeDataDescription gameDataEyeDataDescription) {
        if (mConfig.isDebugOnly() || mConfig.isDebug()) {
            mMessages.postToDebug(gameDataEyeDataDescription);
        } else if (gameDataEyeDataDescription.saveData) {
            mMessages.saveClickData(gameDataEyeDataDescription);
        } else {
            mMessages.postClickData(gameDataEyeDataDescription);
        }
    }
    ...
    private JSONObject obtainDefaultEventProperties(String eventName) {
        ...
        try {
            ...
            finalProperties.put(DataEyeConstants.KEY_NETWORK_TYPE, mGameDataEyeSystemInformation.getNetworkType());

            if(eventName == "app_install"){
                String fit = timeStamp2Date(GameDataEyeSystemInformation.getInstance(mConfig.mContext).getFIT(), "yyyy-MM-dd HH:mm:ss.SSS");
            ...
            if (!TextUtils.isEmpty(mGameDataEyeSystemInformation.getAppVersionName())) {
                finalProperties.put(DataEyeConstants.KEY_APP_VERSION, mGameDataEyeSystemInformation.getAppVersionName());
            }

            final GameDataEyeEventTimer gameDataEyeEventTimer;
            synchronized (mTrackTimer) {
                gameDataEyeEventTimer = mTrackTimer.get(eventName);
                mTrackTimer.remove(eventName);
            }

            if (null != gameDataEyeEventTimer) {
                try {
                    Double duration = Double.valueOf(gameDataEyeEventTimer.duration());
                    if (duration > 0) {
    ...
    public void user_add(String propertyName, Number propertyValue) {
        ...
        try {
            if (null == propertyValue) {
                ...
                if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("Invalid property values for user add.");
            } else {
            ...
            e.printStackTrace();
            if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException(e);
        }
    }
    ...
    private void user_operations(DataEyeConstants.DataType type, JSONObject properties, Date date) {
        ...
        try {
            ...
            trackInternal(new GameDataEyeDataDescription(this, type, finalProperties, time));
        } catch (Exception e) {
    ...
    public void identify(String identity) {
        ...
        if (TextUtils.isEmpty(identity)) {
            ...
            if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("distinct id cannot be empty");
            return;
        }
    ...
    public void reyunAppID(String reyunAppID){
        ...
        if (TextUtils.isEmpty(reyunAppID)) {
            ...
            if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("reyunAppID id cannot be empty");
            return;
        }
...
//        }
    ...
    public void setSuperProperties(JSONObject superProperties) {
        ...
        try {
            if (superProperties == null || !PropertyUtils.checkProperty(superProperties)) {
                if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("Set super properties failed. Please refer to the SDK debug log for details.");
                return;
            }
    ...
    public void timeEvent(final String eventName) {
        ...
        try {
            ...
            synchronized (mTrackTimer) {
                mTrackTimer.put(eventName, new GameDataEyeEventTimer(TimeUnit.SECONDS));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    boolean isActivityAutoTrackAppViewScreenIgnored(Class<?> activity) {
        ...
        GameDataEyeIgnoreTrackAppViewScreenAndAppClick annotation1 =
                activity.getAnnotation(GameDataEyeIgnoreTrackAppViewScreenAndAppClick.class);
        if (null != annotation1 && (TextUtils.isEmpty(annotation1.appId()) ||
                getToken().equals(annotation1.appId()))) {
            return true;
        }

        GameDataEyeIgnoreTrackAppViewScreen annotation2 = activity.getAnnotation(GameDataEyeIgnoreTrackAppViewScreen.class);
    ...
    public void trackViewScreen(Activity activity) {
        ...
        try {
            ...
            if (activity instanceof GameDataEyeScreenAutoTracker) {
                GameDataEyeScreenAutoTracker gameDataEyeScreenAutoTracker = (GameDataEyeScreenAutoTracker) activity;

                String screenUrl = gameDataEyeScreenAutoTracker.getScreenUrl();
                JSONObject otherProperties = gameDataEyeScreenAutoTracker.getTrackProperties();
            ...
            try {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (entry != null) {
                        ...
                        GameDataEyeEventTimer gameDataEyeEventTimer = (GameDataEyeEventTimer) entry.getValue();
                        if (gameDataEyeEventTimer != null) {
                            long eventAccumulatedDuration = gameDataEyeEventTimer.getEventAccumulatedDuration() + SystemClock.elapsedRealtime() - gameDataEyeEventTimer.getStartTime();
                            gameDataEyeEventTimer.setEventAccumulatedDuration(eventAccumulatedDuration);
                            gameDataEyeEventTimer.setStartTime(SystemClock.elapsedRealtime());
                        }
                    }
                }
            } catch (Exception e) {
    ...
    /* package */ void appBecomeActive() {
        GameDataEyeQuitSafelyService.getInstance(mConfig.mContext).start();
        synchronized (mTrackTimer) {
            try {
                Iterator iterator = mTrackTimer.entrySet().iterator();
                while (iterator.hasNext()) {
    ...
    public void enableAutoTrack(List<AutoTrackEventType> eventTypeList) {
        ...
        if (eventTypeList.contains(AutoTrackEventType.APP_CRASH)) {
            mTrackCrash = true;
            GameDataEyeQuitSafelyService quitSafelyService = GameDataEyeQuitSafelyService.getInstance(mConfig.mContext);
    ...
    /* package */ boolean isActivityAutoTrackAppClickIgnored(Class<?> activity) {
        ...
        GameDataEyeIgnoreTrackAppClick annotation2 = activity.getAnnotation(GameDataEyeIgnoreTrackAppClick.class);
    ...
    public void setJsBridge(WebView webView) {
        if (null == webView) {
            ...
            if (mConfig.shouldThrowException()) throw new GameDataEyeDebugException("webView cannot be null for setJsBridge");
            return;
        }

        webView.getSettings().setJavaScriptEnabled(true);
        webView.addJavascriptInterface(new GameDataEyeWebAppInterface(this), "ThinkingData_APP_JS_Bridge");
    }
    ...
    public void setJsBridgeForX5WebView(Object x5WebView) {
        ...
        try {
            ...
            addJavascriptInterface.invoke(x5WebView, new GameDataEyeWebAppInterface(this), "ThinkingData_APP_JS_Bridge");
        } catch (Exception e) {
    ...
    public String getDeviceId() {
        if (mGameDataEyeSystemInformation.getDeviceInfo().containsKey(DataEyeConstants.KEY_DEVICE_ID)) {
            return (String) mGameDataEyeSystemInformation.getDeviceInfo().get(DataEyeConstants.KEY_DEVICE_ID);
        } else {
            return null;
        }
    }
    ...
    /* package */ interface InstanceProcessor {
        void process(GameDataEyeAnalyticsSDK instance);
    }

    /* package */ static void allInstances(InstanceProcessor processor) {
        synchronized (sInstanceMap) {
            for (final Map<String, GameDataEyeAnalyticsSDK> instances : sInstanceMap.values()) {
                for (final GameDataEyeAnalyticsSDK instance : instances.values()) {
    ...
    public void optOutTrackingAndDeleteUser() {
        GameDataEyeDataDescription userDel = new GameDataEyeDataDescription(this, DataEyeConstants.DataType.USER_DEL, null, getTime());
        userDel.setNoCache();
    ...
    private static final GameDataEyeSharedPreferencesLoader sPrefsLoader = new GameDataEyeSharedPreferencesLoader();
    private static Future<SharedPreferences> sStoredSharedPrefs;
    private static final String PREFERENCE_NAME = "com.gamedataeye.analyse";
    ...
    // 缓存 timeEvent 累计时间
    private final Map<String, GameDataEyeEventTimer> mTrackTimer;
    ...
    private GameDataEyeActivityLifecycleCallbacks mLifecycleCallbacks;

    // 保存已经初始化的所有实例对象
    private static final Map<Context, Map<String, GameDataEyeAnalyticsSDK>> sInstanceMap = new HashMap<>();
    ...
    private final GameDataEyeDataHandle mMessages;
    GameDataEyeConfig mConfig;
    private GameDataEyeSystemInformation mGameDataEyeSystemInformation;
 ...
 */
class LightGameDataEyeAnalyticsSDK extends GameDataEyeAnalyticsSDK {
    private String mDistinctId;
    private String mAccountId;
    private final JSONObject mSuperProperties;
    private boolean mEnabled = true;

    LightGameDataEyeAnalyticsSDK(GameDataEyeConfig config) {
        super(config, true);
    ...
    @Override
    public void setNetworkType(GameDataEyeAnalyticsSDK.ThinkingdataNetworkType type) {

    }

    @Override
    public void enableAutoTrack(List<GameDataEyeAnalyticsSDK.AutoTrackEventType> eventTypeList) {
