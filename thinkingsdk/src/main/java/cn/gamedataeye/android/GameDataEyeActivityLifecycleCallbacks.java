...
class GameDataEyeActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    private final GameDataEyeAnalyticsSDK mThinkingDataInstance;
    private final String mMainProcessName;

    private final List<WeakReference<Activity>> mStartedActivityList = new ArrayList<>();

    GameDataEyeActivityLifecycleCallbacks(GameDataEyeAnalyticsSDK instance, String mainProcessName) {
    ...
    private void trackAppStart(Activity activity, ITime time) {
        if (isMainProcess(activity)) {
            if (mThinkingDataInstance.isAutoTrackEnabled()) {
                try {
                    if (!mThinkingDataInstance.isAutoTrackEventTypeIgnored(GameDataEyeAnalyticsSDK.AutoTrackEventType.APP_START)) {
                        ...
                        if (null == time) {
                            ...
                            if (!mThinkingDataInstance.hasDisabled()) {
                                ...
                                GameDataEyeDataDescription gameDataEyeDataDescription = new GameDataEyeDataDescription(mThinkingDataInstance, DataEyeConstants.DataType.TRACK, finalProperties, time);
                                gameDataEyeDataDescription.eventName = DataEyeConstants.APP_START_EVENT_NAME;

                                mThinkingDataInstance.trackInternal(gameDataEyeDataDescription);
                            }
                        }
                    }

                    if (time == null && !mThinkingDataInstance.isAutoTrackEventTypeIgnored(GameDataEyeAnalyticsSDK.AutoTrackEventType.APP_END)) {
    ...
    public void onActivityResumed(Activity activity) {
        ...
        try {
            ...
            if (mThinkingDataInstance.isAutoTrackEnabled() && mShowAutoTrack && !mThinkingDataInstance.isAutoTrackEventTypeIgnored(GameDataEyeAnalyticsSDK.AutoTrackEventType.APP_VIEW_SCREEN)) {
                try {
                    JSONObject properties = new JSONObject();
                    ...
                    if (activity instanceof GameDataEyeScreenAutoTracker) {
                        ...
                        GameDataEyeAutoTrackAppViewScreenUrl autoTrackAppViewScreenUrl = activity.getClass().getAnnotation(GameDataEyeAutoTrackAppViewScreenUrl.class);
            ...
            synchronized (mActivityLifecycleCallbacksLock) {
                ...
                if (mStartedActivityList.size() == 0) {
                    ...
                    if (isMainProcess(activity)) {
                        if (mThinkingDataInstance.isAutoTrackEnabled()) {
                            try {
                                if (!mThinkingDataInstance.isAutoTrackEventTypeIgnored(GameDataEyeAnalyticsSDK.AutoTrackEventType.APP_END)) {
                                    JSONObject properties = new JSONObject();
                                    DataEyeUtils.getScreenNameAndTitleFromActivity(properties, activity);
