...
public class DataHandle {
    ...
    private class SendMessageWorker {
        ...
        private void sendDebugData(TDConfig config, JSONObject data) throws IOException, RemoteService.ServiceUnavailableException, JSONException {
            ...
            ThinkingDataEncrypt encrypt = ThinkingDataEncrypt.getInstance(config.mToken);
            if (encrypt != null && !ThinkingDataEncryptUtils.isEncryptedData(data)) {
...
//            DataEyeLog.i(TAG, "ret code: " + ret + ", upload message:\n" + dataObj.toString(4));
        ...
        private boolean checkResponse(String response) {
            try {
                ...
                TDLog.d(TAG, "checkResponse, responseCode = " + responseCode);
                boolean isSuccess = responseCode == 10000;
                return isSuccess;
            } catch (Exception e) {
                TDLog.d(TAG, "checkResponse, e = " + e.getMessage());
                return false;
            }
        }

        private void checkUploadData(TDConfig config, JSONArray uploadDataArray) {
            for (int i = 0; i < uploadDataArray.length(); i++) {
                try {
                    ...
                    ThinkingDataEncrypt encrypt = ThinkingDataEncrypt.getInstance(config.mToken);
                    if (encrypt != null && ThinkingDataEncryptUtils.isEncryptedData(srcData)) {
                        decryptData = encrypt.decryptTrackData(srcData);
                    }
                    DataDescription.checkDataBeforeUpload(config, decryptData);
        ...
        private void printUploadData(TDConfig config, JSONArray uploadDataArray) {
            for (int i = 0; i < uploadDataArray.length(); i++) {
                try {
                    JSONObject srcData = uploadDataArray.getJSONObject(i);
                    JSONObject decryptData = srcData;
                    ...
                    TDLog.d(TAG, "printUploadData decryptData = " + decryptData);
                } catch (Exception e) {
