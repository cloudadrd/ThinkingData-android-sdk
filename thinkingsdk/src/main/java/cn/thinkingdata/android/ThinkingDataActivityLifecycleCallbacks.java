...
class ThinkingDataActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityStarted(Activity activity) {
        try {
            TDLog.i(TAG, "onActivityStarted: activity = " + activity);
            synchronized (mActivityLifecycleCallbacksLock) {
                ...
                try {
                    if (!mThinkingDataInstance.isAutoTrackEventTypeIgnored(ThinkingAnalyticsSDK.AutoTrackEventType.APP_START)) {
                        ...
                        if (context != null && context instanceof Activity) {
                            TDUtils.getScreenNameAndTitleFromActivity(properties, (Activity) context);
                        }
    ...
    public void onActivityResumed(Activity activity) {
        TDLog.i(TAG, "onActivityResumed: activity = " + activity);
        synchronized (mActivityLifecycleCallbacksLock) {
            if (notStartedActivity(activity, false)) {
    ...
    public void onActivityPaused(Activity activity) {
        TDLog.i(TAG, "onActivityPaused: activity = " + activity);
        synchronized (mActivityLifecycleCallbacksLock) {
            if (notStartedActivity(activity, false)) {
    ...
    public void onActivityStopped(Activity activity) {
        TDLog.i(TAG, "onActivityStopped: activity = " + activity);
        try {
            synchronized (mActivityLifecycleCallbacksLock) {
    ...
    public boolean isAppForegroundWithRunningTask(Context context) {
        ...
        try {
            ...
            TDLog.e(TAG, "isAppForegroundWithRunningTask, Exception = " + e);
        }
        return false;
    }
    ...
    private static boolean isAppForegroundWithRunningAppProcess(Context context) {
        try {
            ...
            TDLog.e(TAG, "isAppForegroundWithRunningAppProcess, Exception = " + e);
        }
        return false;
    }
}
