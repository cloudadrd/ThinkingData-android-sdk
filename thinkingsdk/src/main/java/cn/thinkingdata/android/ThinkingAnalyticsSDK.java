...
import cn.thinkingdata.android.persistence.*;
...
public class ThinkingAnalyticsSDK implements IThinkingAnalyticsAPI {
    ...
    public static ThinkingAnalyticsSDK sharedInstance(TDConfig config) {
        ...
        synchronized (sInstanceMap) {
            ...
            if (null == instances) {
                ...
                QuitSafelyService.getInstance(config.mContext).start();
            }
    ...
    private JSONObject obtainDefaultEventProperties(String eventName) {
        ...
        try {
            ...
            if (eventName == "app_install") {
                String fit = timeStamp2Date(SystemInformation.getInstance(mConfig.mContext).getFIT(), "yyyy-MM-dd HH:mm:ss.SSS");
                finalProperties.put(TDConstants.KEY_FIRST_INSTALL_TIME, fit);

                if (!TextUtils.isEmpty(mReyunAppID.get())) {
                    finalProperties.put(TDConstants.KEY_REYUN_APPID, mReyunAppID.get());
                }
            }
    ...
    public void reyunAppID(String reyunAppID) {
        ...
        if (TextUtils.isEmpty(reyunAppID)) {
            TDLog.w(TAG, "The reyunAppID cannot be empty.");
            if (mConfig.shouldThrowException())
                throw new TDDebugException("reyunAppID id cannot be empty");
            return;
        }
...
//        }
    ...
    /* package */ void appBecomeActive() {
        QuitSafelyService.getInstance(mConfig.mContext).start();
        synchronized (mTrackTimer) {
            try {
                Iterator iterator = mTrackTimer.entrySet().iterator();
                while (iterator.hasNext()) {
    ...
    public void enableAutoTrackLast(List<AutoTrackEventType> eventTypeList) {
        if (eventTypeList.contains(AutoTrackEventType.APP_CRASH)) {
            mTrackCrash = true;
            QuitSafelyService quitSafelyService = QuitSafelyService.getInstance(mConfig.mContext);
