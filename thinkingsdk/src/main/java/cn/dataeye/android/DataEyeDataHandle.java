...
public class DataEyeDataHandle {
     ...
     * 保存数据到本地数据库
     */
    void saveClickData(final DataEyeDataDescription dataDescription) {
     ...
     * 立即上报到服务器，不会缓存和重试
     */
    void postClickData(final DataEyeDataDescription dataDescription) {
     ...
     * Debug 模式上报数据，逐条上报
     */
    void postToDebug(final DataEyeDataDescription dataDescription) {
    ...
    private class SaveMessageWorker {
        ...
        void saveToDatabase(final DataEyeDataDescription dataDescription) {
    ...
    private class SendMessageWorker {
        ...
        // 立即发送数据, 没有重试
        void postToServer(DataEyeDataDescription dataDescription) {
            if (null == dataDescription) return;
            Message msg = Message.obtain();
        ...
        void postToDebug(DataEyeDataDescription dataDescription) {
            if (null == dataDescription) return;
            Message msg = Message.obtain();
