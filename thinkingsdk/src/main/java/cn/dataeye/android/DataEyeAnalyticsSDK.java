...
import cn.dataeye.android.persistence.DataEyeStorageEnableFlag;
import cn.dataeye.android.persistence.DataEyeStorageIdentifyId;
import cn.dataeye.android.persistence.DataEyeStorageLoginID;
import cn.dataeye.android.persistence.DataEyeStorageOptOutFlag;
import cn.dataeye.android.persistence.DataEyeStorageRandomID;
...
public class DataEyeAnalyticsSDK implements DataEyeAnalyticsAPI {
    ...
    DataEyeAnalyticsSDK(DataEyeTDConfig config, boolean... light) {
        ...
        if (null == sStoredSharedPrefs) {
            ...
            sRandomID = new DataEyeStorageRandomID(sStoredSharedPrefs);
            sOldLoginId = new DataEyeStorageLoginID(sStoredSharedPrefs);
        }
        ...
        mLoginId = new DataEyeStorageLoginID(storedPrefs);
        mIdentifyId = new DataEyeStorageIdentifyId(storedPrefs);
        mSuperProperties = new DataEyeStorageSuperProperties(storedPrefs);
        mOptOutFlag = new DataEyeStorageOptOutFlag(storedPrefs);
        mEnableFlag = new DataEyeStorageEnableFlag(storedPrefs);
        ...
        try {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                if (type.isTrack()) {
                    ...
                    dataDescription = new DataEyeDataDescription(this, type, properties, time);
    ...
    void trackInternal(DataEyeDataDescription dataDescription) {
    ...
    private static DataEyeStorageLoginID sOldLoginId;
    private static final Object sOldLoginIdLock = new Object();
    private static DataEyeStorageRandomID sRandomID;
    private static final Object sRandomIDLock = new Object();

    // 本地缓存（SharePreference 相关变量), 单个实例独有. 其文件名称为 PREFERENCE_NAME_{{mToken}}
    private final DataEyeStorageLoginID mLoginId;
    private final DataEyeStorageIdentifyId mIdentifyId;
    private final DataEyeStorageEnableFlag mEnableFlag;
    private final DataEyeStorageOptOutFlag mOptOutFlag;
    private final DataEyeStorageSuperProperties mSuperProperties;
